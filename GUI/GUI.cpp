...
void GUI::CreateMenu() const
{
	...
	MenuItemImages[ITM_LOAD_ROULES] = "GUI\\Images\\Menu\\Menu_load_rules.jpg";
	MenuItemImages[ITM_LOAD_CONCENTRATION] = "GUI\\Images\\Menu\\minor2.jpg";
		...
		while (true)
		{
			...
			if (ctInput == LEFT_CLICK)	//mouse left click
			{
				...
				if (y >= 0 && y < MenuBarHeight)
				{
					...
					case ITM_LOAD_ROULES: return ActionData{ LOAD_RULES2 };		//Exit
					case ITM_LOAD_CONCENTRATION: return ActionData{ LOAD_CONCENTRATION };		//Exit
					break;
