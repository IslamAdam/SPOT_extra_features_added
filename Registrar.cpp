...
//void Registrar::setRegRules(Rules newRules)
//{
//	RegRules = newRules;
//}
...
void Registrar::LoadRules() {
	...
	inFile.close();
	////////////////////////////////////////////////////////////////////////////
	pGUI->PrintMsg("Choose 2nd major: Y =yes , N =no");
	...
	if(dummy =="Y")
{
	...
	while (s6.good())
	{
		...
		{
			if (substr != RegRules.UnivCompulsory[i])
			{
				RegRules.UnivCompulsory.push_back(substr);
				CourseInfo mycourse = getCourseInfo(substr);
				RegRules.ReqUnivCredits = RegRules.ReqUnivCredits + mycourse.Credits;
				RegRules.TotalCredits = RegRules.TotalCredits + mycourse.Credits;
			}
		}
	}
	stringstream s17(lines3[7]);
	while (s7.good())
	{
		string substr;
		getline(s7, substr, ',');
		...
		{
			if (substr != RegRules.UnivElective[i])
			{
				RegRules.UnivElective.push_back(substr);
				CourseInfo mycourse = getCourseInfo(substr);
				RegRules.ElectiveUnivCredits = RegRules.ElectiveUnivCredits + mycourse.Credits;
				RegRules.TotalCredits = RegRules.TotalCredits + mycourse.Credits;
			}
		}
	}
	stringstream s18(lines3[8]);
	while (s8.good())
	{
		string substr;
		getline(s8, substr, ',');
		...
		{
			if (substr != RegRules.TrackCompulsory[i])
			{
				RegRules.TrackCompulsory.push_back(substr);
				CourseInfo mycourse = getCourseInfo(substr);
				RegRules.ReqTrackCredits = RegRules.ReqTrackCredits + mycourse.Credits;
				RegRules.TotalCredits = RegRules.TotalCredits + mycourse.Credits;
			}
		}
	}
	stringstream s19(lines3[9]);
	while (s9.good())
	{
		string substr;
		getline(s9, substr, ',');
		...
		{
			if (substr != RegRules.MajorCompulsory[i])
			{
				RegRules.MajorCompulsory.push_back(substr);
				CourseInfo mycourse = getCourseInfo(substr);
				RegRules.ReqMajorCredits = RegRules.ReqMajorCredits + mycourse.Credits;
				RegRules.TotalCredits = RegRules.TotalCredits + mycourse.Credits;
			}
		}
	}
	stringstream s110(lines3[10]);
	while (s10.good())
	{
		string substr;
		getline(s10, substr, ',');
		...
		{
			if (substr != RegRules.MajorElective[i])
			{
				RegRules.MajorElective.push_back(substr);
				CourseInfo mycourse = getCourseInfo(substr);
				RegRules.ElectiveMajorCredits = RegRules.ElectiveMajorCredits + mycourse.Credits;
				RegRules.TotalCredits = RegRules.TotalCredits + mycourse.Credits;
			}
		}
	}
	...
	/////////////////////////////////////////////////////////////////////////////////
	pGUI->PrintMsg("Declare 2nd concentration: Y =yes , N =no");
	string dummy2 = pGUI->GetSrting();
	if (dummy2 == "Y")
	{
		string line2;
		...
		string conc2 = pGUI->GetSrting();
		stringstream s5(lines2[5]);
		items3.clear();
		while (s5.good())
		{
			string substr;
		...
		if (conc2 == "V")
		{
			...
			RegRules.TotalCredits = RegRules.TotalCredits + Conc_1_Comp + Conc_1_Elec;
		}
		else if (conc2 == "M")
		{
			stringstream s13(lines2[13]);
			while (s13.good())
			{
				string substr;
				getline(s13, substr, ',');
			...
			RegRules.TotalCredits = RegRules.TotalCredits + Conc_2_Comp + Conc_2_Elec;
		}
		else if (conc2 == "P")
		{
			stringstream s15(lines2[15]);
			while (s15.good())
			{
				string substr;
				getline(s15, substr, ',');
			...
			RegRules.TotalCredits = RegRules.TotalCredits + Conc_3_Comp + Conc_3_Elec;
		}
		pGUI->PrintMsg("Importing 2nd Concentration Done Successfully");
	}
	/////////////////////////////////////////////////////////

	pGUI->PrintMsg("Declaring Minor: Y =yes , N =no");
	string dummy3 = pGUI->GetSrting();
	if (dummy3 == "Y")
	{
		ifstream inFile3;
		inFile3.open("E:\\NANENG-Requirements.txt");
		while (getline(inFile, line, '\n'))
		{
			lines.push_back(line);
		}
		...
		for (int i = 0; i < number; i++)
		{
			...
			RegRules.Minor_Courses.push_back(course);
